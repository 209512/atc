...
export const handlers = [
  http.get(`${CONFIG.BASE_URL}/stream`, () => {
    isInitialized = true;
    ...
    return new HttpResponse(new ReadableStream({
      start(controller) {
        const interval = setInterval(() => {
          ...
          const aliveUids = agentList
            .filter(a => !a.isPaused && !simulator.state.globalStop)
            .map(a => a.uuid);
          ...
          const payload = {
            state: { 
              ...
              trafficIntensity: simulator.state.trafficIntensity,
              waitingAgents: aliveUids.filter(id => id !== simulator.state.holder),
              logs: [...simulator.state.logs],
              timestamp: now
            }, 
            agents: agents
          };
  ...
  http.post(`${CONFIG.BASE_URL}/agents/scale`, async ({ request }) => {
  ...
  http.post(`${CONFIG.BASE_URL}/agents/:uuid/pause`, async ({ params, request }) => {
  ...
  http.post(`${CONFIG.BASE_URL}/agents/:uuid/priority`, async ({ params, request }) => {
  ...
  http.post(`${CONFIG.BASE_URL}/agents/:uuid/rename`, async ({ params, request }) => {
  ...
  http.post(`${CONFIG.BASE_URL}/stop`, async ({ request }) => {
    const { enable } = await request.json() as any;
  ...
  http.post(`${CONFIG.BASE_URL}/override`, () => { 
  ...
  http.post(`${CONFIG.BASE_URL}/release`, () => { 
  ...
  http.delete(`${CONFIG.BASE_URL}/agents/:uuid`, ({ params }) => { 
    const uuid = String(params.uuid);
    const agent = simulator.agents.get(uuid);
    if (agent) {
  ...
  http.post(`${CONFIG.BASE_URL}/agents/:uuid/transfer-lock`, ({ params }) => { 
    const uuid = String(params.uuid);
  ...
  http.post(`${CONFIG.BASE_URL}/agents/priority-order`, async ({ request }) => {
  ...
  http.post(`${CONFIG.BASE_URL}/agents/:uuid/config`, async ({ params, request }) => {
